...
function StatusBox(props) {
  ...
  if (healthy) {
    message = (
      <p className={`statusBox__health ${healthy ? "" : "--isDown"}`}>
        Healthy
      </p>
    );
  } else {
  ...
  return (
    <div className="statusBox">
      ...
      <div className="statusBox__body">
        ...
        <p className={`statusBox__hostname ${healthy ? "" : "--isDown"}`}>
          {healthy ? props.status.hostname : "403 forbidden"}
        </p>
  ...
  return (
    <div className="statusBoxes">
      {Object.keys(status).map((key) => {
        return <StatusBox name={key} status={status[key]} />;
      })}
    </div>
  );
}
